...
public class UserManagerBean implements Serializable {
	...
	@PostConstruct
	public void init() {
		connectedUsers = new ArrayList<>(0);
	}
	...
	public Enduser getUserObject() {
		try {
			...
			System.out.println("Exception in getting user object: " + e);
	...
	public boolean isRemoteUserId(int id) {
		try {
			...
			Logger.getLogger(getClass().getName()).info("No remote user: ");
		}
		return false;
	}
	...
	public Future<String> redirectToHome() {
		...
		try {
			...
			return new AsyncResult<>("Session Expired: Redirection done!");
			/*
			 * FacesContext.getCurrentInstance().getExternalContext()
			 * .invalidateSession();
			 */
	...
	/*
	 * <f:selectItem var="_ul" itemValue=
	 * "#{repository.findByCode(userManagerBean.userObject.language.code)}"
	 * itemLabel="#{_ul.label}" />
	 */
	/**
	 * Fetch User Language
	 ...
	 * @return
	 */
	public Language getConnectedUserLanguage() {
		try {
			if (getConnectedUser() == null) {
				return repository.findByCode(FacesContext.getCurrentInstance()
						.getViewRoot().getLocale().getLanguage());
			} else {
				Language l = repository.findByCode(getConnectedUser()
						.getLanguage().getCode());
				return l;
			}
		} catch (Exception ex) {
			Logger.getLogger(UserManagerBean.class.getName()).severe(
					ex.getMessage());
			return null;
		}
	}
	...
	public static void setOriginalPassword(String originalPassword) {
		UserManagerBean.originalPassword = originalPassword;
	}
} // END OF CLASS
