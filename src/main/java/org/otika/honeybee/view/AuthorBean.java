...
public class AuthorBean implements Serializable {
	...
	private Predicate[] getSearchPredicates(Root<Author> root) {
		...
		String namear = this.example.getNamear();
		if (namear != null && !"".equals(namear)) {
			predicatesList.add(builder.like(root.<String> get("namear"),
					'%' + namear + '%'));
		}
		String surnamear = this.example.getSurnamear();
		if (surnamear != null && !"".equals(surnamear)) {
			predicatesList.add(builder.like(root.<String> get("surnamear"),
					'%' + surnamear + '%'));
		}
		String bio = this.example.getBio();
		if (bio != null && !"".equals(bio)) {
			predicatesList.add(builder.like(root.<String> get("bio"),
					'%' + bio + '%'));
		}
