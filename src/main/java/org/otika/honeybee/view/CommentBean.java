...
public class CommentBean implements Serializable
{
   ...
   private Predicate[] getSearchPredicates(Root<Comment> root)
   {
      ...
      if (datecreated != null)
      {
         predicatesList.add(builder.equal(root.get("datecreated"), datecreated));
      }
      
      Date dateupdated = this.example.getDateupdated();
      if (dateupdated != null)
      {
         predicatesList.add(builder.equal(root.get("dateupdated"), dateupdated));
      }
      
      String comcontent = this.example.getComcontent();
      if (comcontent != null && !"".equals(comcontent))
      {
         predicatesList.add(builder.like(root.<String> get("content"), '%' + comcontent + '%'));
      }
      
      String cfname = this.example.getCfname();
      if (cfname != null && !"".equals(cfname))
      {
         predicatesList.add(builder.like(root.<String> get("cfname"), '%' + cfname + '%'));
      }
